...
const BookingItem = ({ item, setRemoveItem, isUpdating, calculateItemPrice }) => {
  return (
    <div className="border-b border-gray-200/50 py-4 last:border-b-0">
      <h3 className="text-lg font-semibold text-gray-900 truncate">{item.test.displayName}</h3>
      <div className="flex flex-col sm:grid sm:grid-cols-2 gap-2 mt-2 text-gray-600 text-sm sm:text-base font-medium">
        <p>
          <strong>Category:</strong> {item.test.testType}
        </p>
        <p>
          <strong>Test Order:</strong> {item.test.orderRequired ? 'Yes' : 'No'}
        </p>
        <p>
      ...
      <div className="flex items-start sm:items-center gap-2 sm:gap-3 mt-3">
        <Button
          ...
          onClick={() => setRemoveItem({ id: item.id, name: item.test.displayName })}
          disabled={isUpdating[item.id]}
          aria-label={`Remove ${item.test.displayName} from booking`}
        >
