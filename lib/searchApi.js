...
export async function searchMedications(params, apiUrl) {
  ...
  // 🔧 FIX: Only include coordinates if NO location filters are applied
  // This prevents sending non-Nigerian coordinates when filtering by state/LGA
  const hasLocationFilters = !params.ignoreFilters && params.filters && 
    (params.filters.state || params.filters.lga || params.filters.ward);

  if (!hasLocationFilters && params.userLocation?.lat && params.userLocation?.lng) {
