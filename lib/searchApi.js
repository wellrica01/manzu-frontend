...
export async function searchMedications(params, apiUrl) {
  ...
  const userLat = params.userLocation?.latitude ?? params.userLocation?.lat;
  const userLng = params.userLocation?.longitude ?? params.userLocation?.lng;
  ...
  if (userLat != null && userLng != null) {
    ...
    if (
      ...
      console.log('📍 Sending coordinates:', { 
        ...
        hasFilters: !!(params.filters?.state || params.filters?.lga)
      });
    } else {
      console.warn('⚠️ Coordinates outside Nigeria, skipping:', { lat, lng });
    }
  } else {
    console.log('ℹ️ No coordinates available for this search');
  }
  ...
  try {
    ...
    // âœ… 4. Validate and normalize the API response
