 ...
 */
import { PrescriptionStatuses } from '../components/cart/cartApi';
...
export function getCartType(segments) {
  ...
  const hasVerified = segments.readyForCheckout.some(
    item => item.medication.prescriptionRequired && item.prescriptionStatus === PrescriptionStatuses.VERIFIED
  );
