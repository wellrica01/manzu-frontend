...
export default function OrderDetails() {
  ...
  const fetchOrders = async (guestId, sessionId) => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/confirmation?session=${sessionId}`, {
        headers: { 'x-guest-id': guestId },
      });
      if (!response.ok) {
        const contentType = response.headers.get('content-type');
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!data.paymentUrl) throw new Error('Payment URL not provided');
      ...
      paystack.newTransaction({
        ...
        onSuccess: (transaction) => {
          router.push(`/confirmation?reference=${transaction.reference}&session=${checkoutSessionId}`);
