...
export default function CategoriesPage() {
  ...
  return (
    <div className="space-y-8">
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Add Category">
        <CategoryForm onSubmit={handleAddCategory} loading={formLoading} error={formError} />
      </Dialog>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-[#225F91]">Categories</h1>
        <button
      ...
      <Card className="p-6 bg-white/95 border border-[#1ABA7F]/20 rounded-2xl shadow-md">
                ...
                {categories.length === 0 ? (
                  ...
                  categories.map((cat) => (
                    <tr key={cat.id} className="border-b last:border-0">
                      <td className="py-2 px-3 font-medium text-gray-900">{cat.id}</td>
        ...
        {deleteSuccess && (
          <div className="fixed bottom-6 right-6 bg-green-100 border border-green-300 text-green-800 px-4 py-2 rounded-lg flex items-center gap-2 shadow-lg z-50">
            <CheckCircle className="w-5 h-5" /> Category deleted successfully.
          </div>
        )}
      </Card>
