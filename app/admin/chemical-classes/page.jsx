...
import { fetchChemicalClasses, deleteChemicalClass, createChemicalClass } from './api';
...
export default function ChemicalClassesPage() {
  ...
  const handleAddChemicalClass = async (data) => {
    setFormLoading(true);
    setFormError(null);
    try {
  ...
  return (
    <div className="space-y-8">
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Add Chemical Class">
        <ChemicalClassForm onSubmit={handleAddChemicalClass} loading={formLoading} error={formError} />
      </Dialog>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {chemicalClasses.length === 0 ? (
                  <tr><td colSpan={3} className="text-center py-4">No chemical classes found.</td></tr>
                ) : (
                  chemicalClasses.map((c) => (
