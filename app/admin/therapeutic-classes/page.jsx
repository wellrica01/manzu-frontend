...
import { fetchTherapeuticClasses, deleteTherapeuticClass, createTherapeuticClass } from './api';
...
export default function TherapeuticClassesPage() {
  ...
  useEffect(() => {
    ...
    fetchTherapeuticClasses()
      .then((res) => {
        setTherapeuticClasses(res.therapeuticClasses || []);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleAddTherapeuticClass = async (data) => {
    setFormLoading(true);
    setFormError(null);
    try {
  ...
  return (
    <div className="space-y-8">
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Add Therapeutic Class">
        <TherapeuticClassForm onSubmit={handleAddTherapeuticClass} loading={formLoading} error={formError} />
      </Dialog>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {therapeuticClasses.length === 0 ? (
                  <tr><td colSpan={3} className="text-center py-4">No therapeutic classes found.</td></tr>
                ) : (
