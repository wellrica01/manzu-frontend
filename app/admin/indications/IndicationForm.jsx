...
export default function IndicationForm({ initialData = {}, onSubmit, loading = false, error = null }) {
  const [form, setForm] = useState({
    genericMedicationId: initialData.genericMedicationId || "",
  ...
  useEffect(() => {
    fetchGenericMedications({ limit: 100 }).then(res => setGenericOptions(res.genericMedications || []));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setFormError(null);
    setSuccess(false);
    if (!form.genericMedicationId || isNaN(Number(form.genericMedicationId))) {
    ...
    try {
      await onSubmit({
        genericMedicationId: Number(form.genericMedicationId),
