...
export default function IndicationsPage() {
  ...
  const handleAddIndication = async (data) => {
    setFormLoading(true);
    setFormError(null);
    try {
      await createIndication(data);
      setDialogOpen(false);
      setRefresh((r) => r + 1);
    } catch (err) {
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      setIndications((prev) => prev.filter((c) => c.id !== id));
      setTimeout(() => {
        setDeleteId(null);
        setDeleteSuccess(false);
      }, 1000);
  ...
  return (
    <div className="space-y-8">
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="Add Indication">
        <IndicationForm onSubmit={handleAddIndication} loading={formLoading} error={formError} />
      </Dialog>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-[#225F91]">Indications</h1>
        <button
      ...
      <Card className="p-6 bg-white/95 border border-[#1ABA7F]/20 rounded-2xl shadow-md">
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-gray-600 border-b">
                  ...
                  <th className="py-2 px-3">Generic Medication ID</th>
              ...
              <tbody>
                {indications.length === 0 ? (
                  <tr><td colSpan={4} className="text-center py-4">No indications found.</td></tr>
                ) : (
                  indications.map((c) => (
                    <tr key={c.id} className="border-b last:border-0">
                      <td className="py-2 px-3 font-medium text-gray-900">{c.id}</td>
                      <td className="py-2 px-3">{c.genericMedicationId}</td>
                      <td className="py-2 px-3">{c.indication}</td>
        ...
        {deleteSuccess && (
          <div className="fixed bottom-6 right-6 bg-green-100 border border-green-300 text-green-800 px-4 py-2 rounded-lg flex items-center gap-2 shadow-lg z-50">
            <CheckCircle className="w-5 h-5" /> Indication deleted successfully.
          </div>
        )}
      </Card>
      ...
      {deleteId && (
        <Dialog open={!!deleteId} onClose={() => setDeleteId(null)} title="Confirm Delete">
          <div className="mb-4">Are you sure you want to delete this indication?</div>
