...
const addUserSchema = z.object({
  ...
  role: z.literal('lab_technician', { errorMap: () => ({ message: 'Role must be lab technician' }) }),
});
...
export default function LabDashboard() {
  ...
  const [labName, setLabName] = useState('Lab Dashboard');
  ...
  const addForm = useForm({
    ...
    defaultValues: { name: '', email: '', password: '', role: 'lab_technician' },
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!decoded.labId || !['lab_manager', 'lab_technician'].includes(decoded.role)) {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      pendingBookings: bookings.filter((booking) => booking.status === 'processing').length,
      completedBookings: bookings.filter((booking) => ['completed', 'results_ready'].includes(booking.status)).length,
      unavailableTests: tests.filter((test) => !test.availability).length,
      totalRevenue: bookings.reduce((sum, booking) => sum + booking.totalPrice, 0),
      recentBookings: bookings.filter((booking) => new Date(booking.createdAt) >= sevenDaysAgo).length,
    };
  }, [bookings, tests]);
  ...
  const fetchLabDetails = async () => {
    try {
      ...
      const data = await response.json();
      setLabName(data.lab?.name || 'Lab Dashboard');
    } catch (err) {
  ...
  const fetchBookings = async () => {
    ...
    try {
      setError(null);
      const token = localStorage.getItem('labToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lab/bookings`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const fetchUsers = async () => {
    ...
    try {
      setUserError(null);
      const token = localStorage.getItem('labToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lab/users`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const updateBookingStatus = async (bookingId, status) => {
    try {
      const token = localStorage.getItem('labToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lab/bookings/${bookingId}`, {
        method: 'PATCH',
        headers: {
      ...
      fetchBookings();
      toast.success('Booking status updated successfully', { duration: 4000 });
      setShowDialog({ open: false, type: '', data: null });
      if (typeof window !== 'undefined' && window.gtag) {
        window.gtag('event', 'update_booking_status', { bookingId, status });
      }
    } catch (err) {
      setError(err.message);
      toast.error(err.message, { duration: 4000 });
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (filterStatus === 'pending') {
      result = result.filter((booking) => booking.status === 'processing');
    }
  ...
  const handleAddUser = async (values) => {
    try {
      setUserError(null);
      const token = localStorage.getItem('labToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lab/auth/add-user`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ ...values, labId }),
      });
      const data = await response.json();
      if (!response.ok) {
  ...
  const handleEditUser = async (values, userId) => {
    try {
      setUserError(null);
      const token = localStorage.getItem('labToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lab/auth/users/${userId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const getNextStatuses = (status, sampleCollectionMethod) => {
    const nextStatuses = {
      confirmed: ['processing'],
      processing: sampleCollectionMethod === 'in_lab' ? ['results_ready'] : ['sample_collected'],
      sample_collected: ['results_ready'],
          ...
          <CardContent className="p-6 sm:p-10 space-y-8">
            <div className="grid grid-cols-1 gap-8">
              <div className="space-y-4">
                <h3 className="text-lg font-bold text-primary tracking-wide">Bookings</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div
                    ...
                    aria-label={`Total Bookings: ${metrics.totalBookings}`}
                    onClick={() => console.log('Navigate to all bookings')}
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-semibold text-gray-700 mt-1">Total Bookings</p>
                      </div>
                    </div>
                  </div>
                  <div
                    ...
                    aria-label={`Pending Bookings: ${metrics.pendingBookings}`}
                    onClick={() => console.log('Navigate to pending bookings')}
                  >
                    <div className="flex items-center gap-5">
                      <div className="relative">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-semibold text-gray-700 mt-1">Pending Bookings</p>
                      </div>
                    </div>
                  </div>
                  <div
                    ...
                    aria-label={`Completed Bookings: ${metrics.completedBookings}`}
                    onClick={() => console.log('Navigate to completed bookings')}
                  >
                    <div className="flex items-center gap-5">
                      <div className="relative">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-semibold text-gray-700 mt-1">Completed Bookings</p>
                      </div>
                    </div>
                  </div>
                  <div
                    ...
                    aria-label={`Recent Bookings: ${metrics.recentBookings}`}
                    onClick={() => console.log('Navigate to recent bookings')}
                  >
                    <div className="flex items-center gap-5">
                      <div className="relative">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-semibold text-gray-700 mt-1">Recent Bookings (7d)</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-bold text-primary tracking-wide">Tests</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
                  <div
                    className="p-6 rounded-2xl bg-gradient-to-br from-white/95 to-gray-50/95 border-2 border-gradient-to-r from-primary/30 to-blue-600/30 backdrop-blur-md cursor-pointer transition-all duration-300 hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-primary/50 animate-in fade-in-20"
                    style={{ animationDelay: '0.5s' }}
                    aria-label={`Unavailable Tests: ${metrics.unavailableTests}`}
                    onClick={() => console.log('Navigate to unavailable tests')}
                  >
                    <div className="flex items-center gap-5">
                      <div className="relative">
                      ...
                      <div className="flex-1">
                        <p className="text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-600 truncate">
                          {metrics.unavailableTests}
                        </p>
                        <p className="text-sm font-semibold text-gray-700 mt-1">Unavailable Tests</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div
                    ...
                    onClick={() => console.log('Navigate to revenue overview')}
                  >
                    <div className="flex items-center gap-5">
                      <div className="relative">
        ...
        >
          ...
          <CardContent className="p-6 sm:p-8 space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              <Input
                placeholder="Search by Booking ID or Tracking Code"
              ...
              >
                ...
                <SelectContent className="bg-white/95 backdrop-blur-md border-gray-100/30 rounded-2xl">
                  <SelectItem value="pending">Pending Bookings</SelectItem>
            ...
            {loading ? (
              <div className="text-center py-12">
                <Loader2
                  ...
                  aria-hidden="true"
                />
                <p className="text-gray-600 text-lg font-medium mt-4">Loading bookings...</p>
              </div>
            ) : (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50/70 border-b border-gray-100/20">
                      <TableHead className="text-primary text-sm font-bold uppercase tracking-wider py-4">Booking ID</TableHead>
                      ...
                      <TableHead className="text-primary text-sm font-bold uppercase tracking-wider py-4">Collection</TableHead>
                      <TableHead className="text-primary text-sm font-bold uppercase tracking-wider py-4">Tests</TableHead>
                  ...
                  <TableBody>
                    {filteredBookings.length === 0 ? (
                      <TableRow>
                        ...
                        >
                          No {filterStatus === 'pending' ? 'pending' : ''} bookings found.
                        </TableCell>
                      </TableRow>
                    ) : (
                      filteredBookings.map((booking, index) => (
                        ...
                        >
                          <TableCell className="text-base font-medium text-gray-900 py-4">
                            <button
                              onClick={() => setShowDialog({ open: true, type: 'bookingDetails', data: booking })}
                          ...
                          <TableCell className="text-base font-medium text-gray-900 py-4">
                            {booking.sampleCollectionMethod === 'in_lab' ? 'In-Lab' : 'Home Collection'}
                          </TableCell>
                          <TableCell
                            className="text-base font-medium text-gray-900 truncate max-w-[180px] py-4"
                            title={booking.tests.map((test) => test.name).join(', ')}
                          >
                            {booking.tests.map((test) => test.name).join(', ')}
                          </TableCell>
                          <TableCell className="text-base font-medium text-gray-900 py-4">
                            {booking.totalPrice.toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })}
                          </TableCell>
                          <TableCell className="py-4">
                            <span
                              ...
                              aria-label={`Booking status: ${booking.status.replace('_', ' ').toUpperCase()}`}
                            >
                            ...
                            <Select
                              onValueChange={(value) =>
                                ...
                                setShowDialog({
                                  open: true,
                                  type: 'updateStatus',
                                  data: { bookingId: booking.id, status: value },
                                })
                              }
                              disabled={!getNextStatuses(booking.status, booking.sampleCollectionMethod).length}
                            >
                              <SelectTrigger
                              ...
                              <SelectContent className="bg-white/95 backdrop-blur-md border-gray-100/30 rounded-xl shadow-lg">
                                {getNextStatuses(booking.status, booking.sampleCollectionMethod).map((status) => (
                                  <SelectItem
                                    key={status}
                                    value={status}
                        ...
                        >
                          {editingUserId === user.id ? (
                            ...
                            <>
                              ...
                              <TableCell className="text-base font-medium text-gray-900">
                                {user.role === 'lab_manager' ? 'Lab Manager' : 'Lab Technician'}
                              </TableCell>
                              <TableCell>
              ...
              {showDialog.type === 'addUserForm' ? (
                ...
                <p className="text-base text-gray-600 font-medium">
                  ...
                  {showDialog.type === 'updateStatus' &&
                    `Update booking #${showDialog.data?.bookingId} status to ${showDialog.data?.status
                      .replace('_', ' ')
                      .toUpperCase()}?`}
                </p>
              )}
                ...
                <Button
                  onClick={() => {
                    ...
                    if (showDialog.type === 'updateStatus')
                      updateBookingStatus(showDialog.data.bookingId, showDialog.data.status);
                  }}
                  className="h-12 px-6 text-sm font-semibold rounded-full bg-primary text-white hover:bg-primary/90 hover:shadow-[0_0_15px_rgba(59,130,246,0.5)] animate-pulse transition-all duration-300"
